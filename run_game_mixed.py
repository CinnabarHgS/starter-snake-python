...
def build_cmd(seed: int, names: list[str]):
    cmd = [
...
def main(seed: int = 123, names: list[str] | None = None) -> bool:
    if names is None:
        names = ["Candidate", "Base1", "Base2", "Base3"]
    ...
    try:
        proc = subprocess.Popen(build_cmd(seed, names))
